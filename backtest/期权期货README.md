...
cash为初始资金数量的浮点数
后置条件：
返回推荐选择期货的信息表：Dataframe

#### 新增部分

#### rename_beta_model(protfolio_id<str>)
#### rename_delta_model(protfolio_id<str>)
#### rename_gamma_model(protfolio_id<str>)
将临时模型转存为保存的模型
前置条件：
protfolio_id 为组合的id